...
- [x] **Prasyarat:** pastikan v1.1.0 (dengan AC-10.2) sudah dirilis sebelum `schemaVersion` dinaikkan — *baseline M12 adalah tag `v1.1.0` (commit 717edcd); guard AC-10.2 sudah ada di `BackupService.validateBackupFile` sejak M11 dan diuji ulang dua arah dengan skema baru*
- [x] Tabel Drift baru `customers` & `customer_point_entries`, kolom `sales.customer_id`, plus 3 index sesuai PRD §7.5 (`idx_customers_name_nocase` parsial untuk pelanggan aktif, `idx_sales_customer`, `idx_point_entries_customer`)
- [x] `schemaVersion` 1 → 2 dengan `onUpgrade` **backfill** PRD §7.3.E dalam satu transaksi: `DISTINCT TRIM(customer_name)`, dikelompokkan case-insensitive, ejaan terbanyak menang (seri → paling awal), `customer_id` diisi untuk semua transaksi terkait. `sales.customer_name` **tidak dihapus & tidak diubah** (K-7.1, AC-10.4, AC-10.5)
- [x] Entity + kontrak `CustomerRepository` + implementasi Drift + provider; key `settings` baru: `points_enabled` (default `0`), `points_rupiah_per_point` (`10000`), `points_value_per_point` (`500`), `points_min_redeem` (`10`)
- [x] Buku besar poin (K-7.2) dengan tipe `earn` / `redeem` / `void_return` / `adjust` / `merge`; `customers.points` hanyalah cache yang diperbarui **di transaksi DB yang sama**; poin bilangan bulat (K-7.3); **tidak ada poin surut** (K-7.4)
- [x] Integrasi `SaveSaleUsecase`: poin dihitung dari total **setelah diskon** dan **tidak termasuk** potongan hasil penukaran poin (AC-7.10); berlaku untuk semua metode bayar termasuk hutang (K-7.5)
- [x] Integrasi `VoidSaleUsecase`: poin ditarik kembali & poin yang sempat ditukar dikembalikan, keduanya sebagai entri ledger terpisah dalam transaksi void yang sama; saldo tidak boleh negatif (dipatok 0 + entri bercatatan jelas)
- [x] Penukaran poin di sheet pembayaran → **diskon level transaksi** pada `sales.discount` + entri `redeem` (K-7.6); baris "Tukar poin" muncul di struk teks **dan** struk ESC/POS (M8)
- [x] Pemilih pelanggan menggantikan field teks bebas: bottom sheet dengan pencarian autofocus, baris ≥ 56dp, opsi "Buat pelanggan baru: <ketikan>", wajib untuk hutang (`NamaPelangganWajibException` dipertahankan, AC-7.4), dan **nol tap tambahan** bila pelanggan tidak dipilih (AC-7.5)
- [x] Layar **Pelanggan** diakses dari tab Laporan (kartu "Hutang Pelanggan" berkembang jadi kartu "Pelanggan"); `debt_list_screen.dart` menjadi filter "Punya hutang" di dalamnya. **Navigasi bawah tetap 5 tab**
- [x] Detail pelanggan: ringkasan (total belanja, jumlah transaksi, sisa hutang, saldo poin), riwayat belanja berpaginasi (pola `AsyncNotifier` M3), riwayat poin (`AppDataRow` +/−); aksi ubah, nonaktifkan (ditolak bila masih berhutang, AC-7.13)
- [x] Gabungkan pelanggan (PRD §7.3.D): mode pilih, pratinjau dampak, satu transaksi DB, sumber ditandai nonaktif + `merged_into_id`, **satu arah & tidak bisa dibatalkan** (K-7.7)
- [x] Pengaturan → kartu "Program Poin" (**default mati**, seluruh UI poin tersembunyi saat mati) + aksi pemeliharaan "hitung ulang saldo dari ledger"
- [x] Nama pelanggan di detail transaksi menjadi tautan ke profil pelanggan
- [x] Export Excel mendapat sheet/berkas "Pelanggan & Poin" (nama, no. HP, total belanja, sisa hutang, saldo poin) — AC-7.16
- [x] Seluruh layar baru memakai `context.palette`; dilarang `AppColors.*`
- [x] **Test migrasi atas snapshot database v1 nyata** (AC-10.1): `"Bu Ani"` / `"bu ani"` / `"Bu Ani "` → satu pelanggan dengan tiga transaksi (AC-7.1); total hutang sebelum vs sesudah **identik** (AC-7.2); `customer_name` lama tidak berubah setelah rename (AC-7.3)
- [x] Test invarian: `customers.points` **selalu** sama dengan jumlah entri ledger setelah rangkaian acak jual/void/tukar/gabung (AC-7.11); penggabungan 3 pelanggan tidak menghilangkan entri (AC-7.12); aturan perolehan Rp37.000 → 3 poin, Rp9.999 → 0 poin (AC-7.7); program mati → nol elemen poin di layar mana pun termasuk struk (AC-7.6)
- [x] Test performa: pencarian pada 2.000 pelanggan < 100 ms (AC-7.14); detail pelanggan dengan 5.000 transaksi tetap mulus (AC-7.15) — *diukur di mesin pengembang dengan ambang longgar (< 500 ms) + uji `EXPLAIN QUERY PLAN` yang mengunci pemakaian index; angka 100 ms sesungguhnya butuh perangkat fisik*
- [ ] Uji manual device fisik: alur kasir tunai tanpa memilih pelanggan tidak bertambah satu tap pun; **restore backup v1.0 di perangkat lain → migrasi jalan otomatis & total hutang sama persis** — *BUTUH PERANGKAT FISIK; padanan otomatisnya sudah hijau (uji migrasi atas snapshot v1 & smoke test alur kasir tunai)*
...
- [x] Tabel Drift baru `users`; `ALTER TABLE` menambah `sales.user_id`, `sales.user_name`, `sales.voided_by_user_id`, `stock_movements.user_id`; index `idx_users_name_nocase` (parsial, aktif saja) & `idx_sales_user` (PRD §8.5)
- [x] `schemaVersion` 2 → 3, seluruhnya `ADD COLUMN` (O(1) di SQLite, non-destruktif); indikator progres bila pembuatan index > 1 detik
- [x] Key `settings` baru: `multi_user_enabled` (**default `0`**), `auto_lock_minutes` (`0` = mati), `recovery_code_hash`, `recovery_code_salt`. Key lama `pin_hash`/`pin_salt` **dipertahankan** untuk mode single-user
- [x] `UserRepository` + reuse `PinHasher` (SHA-256) dengan **salt per pengguna** (K-8.3); tidak ada PIN teks polos di DB maupun `shared_preferences` (AC-8.14). Sesi aktif disimpan sebagai `active_user_id` di `shared_preferences`
- [x] Alur aktivasi (PRD §8.3.A): PIN global yang sudah ada **otomatis** menjadi PIN akun "Pemilik" (AC-8.2); bila belum ada, buat PIN Pemilik 6 digit
- [x] **Kode pemulihan 8 karakter** ditampilkan **sekali saja**, disimpan sebagai hash, dengan tombol Salin/Bagikan dan centang wajib "Saya sudah mencatat" (K-8.4, AC-8.3)
- [x] Layar **Masuk** sebagai rute di luar shell navigasi: kartu pengguna ≥ 64dp (avatar inisial) → keypad PIN me-*reuse* `pin_keypad.dart` / `pin_entry_screen.dart`; judul "Siapa yang bertugas?". Pilih nama dulu, baru PIN (K-8.2, AC-8.15)
- [x] Dua peran tetap (Pemilik / Kasir) dengan matriks izin PRD §8.3.C — **tidak bisa dikustomisasi** (K-8.1)
- [x] Penjagaan izin di **lapisan `redirect` go_router sekaligus di UI** (AC-8.4); elemen berisi laba & harga modal **disembunyikan sepenuhnya** untuk Kasir, bukan diburamkan (AC-8.5); Kasir hanya melihat Riwayat hari ini dan tidak bisa void (AC-8.6)
- [x] Penolakan akses memakai pola `EmptyState` (ikon gembok + kalimat pengarah + tombol "Masuk sebagai Pemilik"), bukan dialog error telanjang
- [x] "Ganti Kasir" di Pengaturan dan menu ⋮ layar Kasir (satu-satunya tambahan di layar kasir) + chip pengguna aktif di AppBar yang tidak boleh lebih menonjol dari CTA "Bayar"
- [x] Kunci otomatis opsional (mati / 1 / 5 / 15 menit); **keranjang berjalan tidak pernah dibuang** — hanya ditutupi layar PIN (AC-8.11, AC-8.12)
- [x] Rate limit: 5 PIN salah → keypad terkunci 30 detik, berlipat sampai maksimal 5 menit, dan **bertahan setelah aplikasi ditutup-buka** (AC-8.10). Tidak ada penghapusan data
- [x] Jejak pengguna: `user_id` + `user_name` snapshot di setiap penjualan (K-8.6), `user_id` di setiap penyesuaian stok, `voided_by_user_id` saat void; baris `Kasir: <nama>` di struk teks & ESC/POS (M8)
- [x] Filter "Kasir" di Riwayat dan Laporan (AC-8.9)
- [x] Reset PIN kasir oleh Pemilik; pemulihan PIN Pemilik lewat kode pemulihan (kode lama hangus, kode baru diterbitkan)
- [x] Mematikan multi-user: akun kasir dinonaktifkan, PIN Pemilik kembali jadi PIN global, `sales.user_id` historis **tetap dipertahankan** (AC-8.13)
- [x] Test migrasi 2 → 3 atas snapshot database v2 (AC-10.1); test per peran termasuk percobaan akses rute langsung; test multi-user mati = perilaku v1.0 persis (AC-8.1); test nama pengguna diganti tidak mengubah `user_name` transaksi lama (AC-8.7, AC-8.8)
- [ ] Uji manual device fisik: ganti kasir saat keranjang berisi, kunci otomatis lalu buka kembali, rasa keypad PIN, restore backup schema 3 membawa seluruh akun & meminta masuk (AC-8.16) — *belum dicentang: butuh perangkat fisik; padanan otomatisnya hijau (uji restore backup schema 3 & uji gerbang masuk), lihat [laporan-m13.md](laporan-m13.md) §8*
...
### Milestone 12 — Pelanggan & Program Poin

- **K-7.8 (baru) — `PointLedger` adalah satu-satunya penulis saldo poin.** PRD K-7.2 menetapkan bahwa `customers.points` cuma cache dan buku besarnya sumber kebenaran, tapi tidak menetapkan *siapa* yang boleh menulis keduanya. Dibiarkan terbuka, penulisnya akan ada tiga (jual, void, gabung) dengan tiga cara memperbarui cache — dan invarian AC-7.11 hanya soal waktu sebelum melenceng. `data/db/point_ledger.dart` karena itu menjadi satu-satunya jalan masuk; ia sengaja **tidak** membuka transaksi sendiri supaya penulisan poin selalu atomik bersama transaksi yang memicunya.
- **K-7.9 (baru) — pematokan saldo ke 0 mengubah nilai ENTRI, bukan cuma nilai saldo.** PRD §7.3.C meminta "saldo dipatok 0 dan entri buku besar mencatat selisihnya". Ditafsir harfiah — entri mencatat −5 padahal saldo cuma turun 2 — invarian AC-7.11 pecah pada langkah pertama. Yang dilakukan: entri mencatat **poin yang benar-benar terpakai** (−2), dan selisih yang diabaikan dijelaskan di kolom `note` ("Saldo dipatok 0: 3 poin tidak bisa ditarik karena sudah terpakai di transaksi lain"). Buku besar tetap bisa dijumlahkan; keterangannya tetap lengkap.
- **K-7.10 (baru) — "hitung ulang saldo dari buku besar" MEMPERBAIKI CACHE, bukan menambah entri koreksi.** Godaannya besar untuk menulis entri `adjust` sebesar selisih agar jejaknya terlihat — dan itu justru salah secara aritmetika: entri koreksi ikut menggeser jumlah buku besar, sehingga cache mengejar target yang bergerak dan invariannya tidak pernah bertemu (ini benar-benar terjadi, tertangkap uji AC-7.11). Aksi pemeliharaan karena itu menulis `points = SUM(ledger)` dan meninggalkan entri `adjust` bernilai **0 poin** sebagai penanda beserta angka sebelum/sesudahnya di `note`. Alasan yang sama membuat entri `merge` juga bernilai 0 poin.
- **K-7.11 (baru) — void mengembalikan poin yang ditukar LEBIH DULU, baru menarik poin yang didapat.** Urutan ini tidak disebut PRD, tapi menentukan hasilnya: menarik dulu berarti saldo sempat menyentuh 0 dan pematokan K-7.9 aktif walau sebenarnya poinnya masih ada di transaksi itu sendiri. Dengan urutan ini, pematokan hanya terjadi bila poinnya memang sudah terpakai di transaksi **lain** — yaitu satu-satunya kasus yang PRD maksud.
- **`sales.customer_name` kini ditulis untuk SEMUA metode bayar, bukan hanya hutang.** Di v1.0 kolom itu hanya terisi pada transaksi hutang. Sejak M12 pelanggan boleh dipilih pada transaksi tunai/non-tunai juga, dan snapshot namanya tetap ditulis (K-7.1) supaya struk tunai pun tidak berubah saat pelanggan di-*rename*. Tidak ada laporan lama yang terpengaruh: seluruhnya menyaring `status = 'debt_unpaid'` lebih dulu.
- **`debt_list_screen.dart` & `customer_debt_transactions_screen.dart` DIHAPUS, bukan disembunyikan.** PRD §7.6 memerintahkan daftar hutang menjadi filter di dalam daftar pelanggan. Menyisakan layar lamanya berarti dua daftar hutang yang mengelompokkan dengan aturan berbeda — yang lama per teks nama, yang baru per `customer_id` — dan yang lama akan tetap menampilkan "Bu Ani" & "bu ani" sebagai dua penghutang. Method `ReportRepository.getUnpaidDebts`/`getDebtTransactions` **tetap ada** (agregasi per snapshot nama historis, masih diuji test M4) tapi kini tanpa konsumen UI; keduanya tidak dihapus supaya test M0–M6 tidak perlu diubah, dan alasannya ditulis sebagai komentar di `report_providers.dart`.
- **Poin ditampilkan di struk sebagai baris terpisah dari diskon transaksi.** Penukaran poin disimpan sebagai `sales.discount` (K-7.6), tapi mencetaknya sebagai satu angka "Diskon transaksi" membuat pembeli tidak pernah melihat poinnya terpakai — padahal itu seluruh alasan program poin ada. Nilai penukaran karena itu dikurangkan dari diskon yang dicetak dan diberi barisnya sendiri. Untuk transaksi yang dibuka ulang dari Riwayat, nilai rupiahnya disimpulkan dari `discount ÷ poin`; bila tidak habis dibagi (ada diskon manual juga), barisnya tetap muncul **tanpa nominal** dan angka totalnya tetap konsisten.
- **Empat kolom ringkasan detail pelanggan dibungkus `IntrinsicHeight`.** `CrossAxisAlignment.stretch` di dalam `ListView` melempar "BoxConstraints forces an infinite height" — tertangkap smoke test HP 392dp, bukan oleh mata. Tanpa itu, dua kartu dalam satu baris tampak "patah" begitu salah satu nominalnya jauh lebih panjang.
- **Uji migrasi memakai fixture DDL yang disalin dari `sqlite_master` v1 sungguhan** (`test/fixtures/v1_database_fixture.dart`), bukan skema tiruan. Uji migrasi di atas skema "kira-kira mirip" tidak membuktikan apa pun tentang database pengguna; fixture ini juga sengaja diisi data berantakan (tiga ejaan satu nama, nama kosong, `NULL`, transaksi voided, hutang lunas) karena itulah bentuk database warung yang sebenarnya.
...
### Milestone 13 — Multi-User dengan PIN per Kasir

- **K-8.8 (baru) — migrasi 2 → 3 MEMBUAT akun Pemilik dari PIN global lama, tapi TIDAK menyalakan multi-user.** PRD §8.3.A menulis pemindahan PIN itu sebagai bagian alur "aktifkan multi-user", sedangkan AC-8.1 mewajibkan aplikasi berperilaku persis v1.0 selama fiturnya mati. Dua-duanya dipenuhi dengan memisahkan *penyiapan* dari *penyalaan*: migrasi menyalin `settings.pin_hash`/`pin_salt` menjadi akun `Pemilik` (hash-nya, bukan PIN-nya), `multi_user_enabled` tetap tak terisi, dan key PIN lama dibiarkan utuh sehingga gerbang PIN v1.0 tetap yang berlaku. Efeknya: pemilik yang sudah memasang PIN sejak M5 menyalakan multi-user tanpa pernah diminta membuat PIN baru (AC-8.2), dan warung yang belum pernah memasang PIN tidak mendapat akun tanpa PIN.
- **K-8.9 (baru) — kasir aktif disuntikkan ke usecase lewat KONSTRUKTOR, bukan parameter pemanggilan.** `SaveSaleUsecase`/`VoidSaleUsecase`/`AdjustStockUsecase` menerima `actor` dari providernya (`ref.watch(currentUserProvider)`). Alternatifnya — menambah parameter `userId` di setiap pemanggilan — berarti setiap layar baru bisa lupa mengisinya, dan jejak yang bolong sama saja dengan tidak ada jejak (AC-8.7). Bonusnya: seluruh test M0–M12 yang membangun usecase tanpa `actor` tetap kompilasi dan tetap benar, karena `null` memang arti "multi-user mati".
- **K-8.10 (baru) — peran yang TIDAK DIKETAHUI diperlakukan sebagai Kasir, bukan Pemilik.** Berlaku di dua tempat: `UserRole.fromDb` untuk nilai `role` asing di database, dan `SessionState.role` selama akun aktif belum selesai dimuat dari database pada frame-frame pertama. Ketidaktahuan tidak boleh pernah menjadi izin — kalau jatuh ke Pemilik, ada jendela beberapa frame di mana rute Laporan terbuka untuk siapa pun.
- **K-8.11 (baru) — `multi_user_enabled` dicerminkan ke `shared_preferences`.** Sumber kebenarannya tetap tabel `settings` (supaya ikut backup/restore, AC-8.16), tapi `redirect` go_router butuh jawaban **sinkron** di frame pertama, sedangkan membaca database berarti menunggu. Cermin dibaca untuk frame pertama lalu langsung diselaraskan dengan database; alasannya sama persis dengan tema (AC-5.5) — tanpa itu, warung multi-user melihat kedipan layar Kasir sebelum layar Masuk.
- **K-8.12 (baru) — `checkPinGate` menjadi no-op saat multi-user aktif.** Gerbang PIN global v1.0 masih menjaga tab Laporan/Pengaturan/void di mode single-user. Membiarkan keduanya hidup bersama berarti kasir dimintai PIN global *lalu* ditolak karena perannya — dua penghalang untuk satu aksi, dan dorongan nyata bagi kasir untuk menghafal PIN pemiliknya.
- **Rute `/pengaturan` sengaja TIDAK masuk daftar rute Pemilik.** §8.3.C memberi Kasir akses tema. Yang dijaga karena itu adalah isinya: `SettingsScreen` merender badan yang berbeda per peran (Kasir hanya melihat kartu sesi + Tampilan), bukan layar yang sama dengan tombol-tombol mati. Rute `/laporan` dan `/produk/**` (form tambah/ubah) tetap ditolak di `redirect`.
- **Penolakan akses adalah RUTE (`/akses-ditolak`), bukan dialog.** Konsekuensi langsung dari "penjagaan di lapisan router": redirect harus punya tujuan. Layarnya memakai pola `EmptyState` dengan dua jalan keluar ("Masuk sebagai Pemilik", "Kembali ke Kasir"), sesuai §8.6 dan supaya kasir tidak berakhir di layar buntu di depan pembeli.
- **Kode pemulihan memakai abjad Crockford tanpa `I`/`L`/`O`/`U`, dan normalisasinya memaafkan `O`↔`0` & `I`/`L`↔`1`.** Kode ini akan disalin dengan tangan ke kertas lalu diketik ulang berbulan-bulan kemudian — alasan yang sama persis dengan kode lisensi §6. Kode yang benar-benar dicatat pengguna tidak boleh ditolak hanya karena tulisan tangannya terbaca `O` alih-alih `0`.
- **Batas "riwayat hari ini" untuk Kasir dipasang di provider, bukan di sheet filter.** `HistoryListNotifier._fetch` menjepit rentang tanggalnya sesuai peran sebelum query dikirim. Dipasang di sheet, batas itu bisa tertinggal longgar dari sesi pemilik sebelumnya — dan filter yang sudah terlanjur terbuka adalah kebocoran yang tidak terlihat siapa pun.
